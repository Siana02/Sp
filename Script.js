...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function animateCounter(counter) {
    ...
    const duration = 2000; // 2 seconds for all counters, regardless of value
    const startValue = 0;
    ...
    function updateCounter(currentTime) {
      ...
      // Only show suffix when finished, otherwise keep suffix visible during animation
  ...
  // Only animate once when .belt comes into view
    ...
    const observer = new IntersectionObserver((entries, observer) => {
      entries.forEach(entry => {
        if (entry.isIntersecting && !hasAnimated) {
          ...
          observer.unobserve(beltSection);
        }
      });
    }, { threshold: 0.3 });
